...
class PPO:
    ...
    def act(self, obs: TensorDict) -> torch.Tensor:
        ...
        # 记录当前隐藏状态（用于循环策略）。
        # 调用 actor(obs, stochastic_output=True) 采样动作。
        # 调用 critic(obs) 计算价值。
        # 获取动作的对数概率和分布参数。
        # 将观测保存到 transition 中，并返回动作。
    ...
    def process_env_step(
        ...
        # 更新归一化器（如果模型使用了归一化）。
        # 克隆奖励和 done 标志（rewards 和 dones）。
        # 如果启用了 RND，计算内在奖励并加到外部奖励上。
        # 如果 extras 中包含 "time_outs"，则对超时步进行自举（bootstrap），将未来价值加到当前奖励上（处理无限视野任务）。
        # 将 transition 添加到 storage 中，并清除 transition。
        # 根据 dones 重置模型的隐藏状态（用于循环策略）。
    ...
    def update(self) -> dict[str, float]:
        ...
        for batch in generator:
            ...
            # Surrogate loss
            # 优势混合
            mixing_advantages_batch = torch.zeros_like(batch.advantages)
            mixing_advantages_batch[..., 0] = batch.advantages[..., 0] + value_mixing_ratio * batch.advantages[..., 1]      # type: ignore
            mixing_advantages_batch[..., 1] = batch.advantages[..., 1] + value_mixing_ratio * batch.advantages[..., 0]      # type: ignore
