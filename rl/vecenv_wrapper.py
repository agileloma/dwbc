...
class B2Z1RslRlVecEnvWrapper(VecEnv):
  def __init__(
    self,
    ...
    self.num_prop = env.unwrapped.num_prop 
    self.num_priv = env.unwrapped.num_priv
    self.num_history = env.unwrapped.num_history
  ...
  @property
  def unwrapped(self) -> B2Z1ManagerRLEnv:
    return self.env.unwrapped
  ...
  #   term_or_trunc = terminated | truncated
  #   assert isinstance(rew, torch.Tensor)
  #   assert isinstance(arm_rew, torch.Tensor)
  #   assert isinstance(term_or_trunc, torch.Tensor)
  #   dones = term_or_trunc.to(dtype=torch.long)
  #   if not self.cfg.is_finite_horizon:
  #     extras["time_outs"] = truncated
  ...
  def step(self, actions: torch.Tensor) -> tuple[TensorDict, torch.Tensor, torch.Tensor, torch.Tensor, dict]:
    if self.clip_actions is not None:
        actions = torch.clamp(actions, -self.clip_actions, self.clip_actions)
    obs_dict, rew, arm_rew, dones, extras = self.env.step(actions)
    return (
        TensorDict(obs_dict, batch_size=[self.num_envs]),
        rew,
        arm_rew,
